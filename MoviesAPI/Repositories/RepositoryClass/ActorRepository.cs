using System.Collections.Generic;
using System.Linq;
using Microsoft.EntityFrameworkCore;
using MoviesAPI.Models;
using MoviesAPI.Repositories.RepositoryInterface;

namespace MoviesAPI.Repositories.RepositoryClass
{
    public class ActorRepository : Repository<Actor> , IActorRepository
    {
        public ActorRepository(MoviesDbContext context)
            : base(context)
        {

        }

        public MoviesDbContext  DbContext
        {
            get
            {
                return Context as MoviesDbContext;
            }
        }

        public IEnumerable<MovieActor> GetActorsByMovie(int i_SelectedMovieId)
        {
            return DbContext.MoviesAndActors.Include(ma => ma.Actor).Where(ma => ma.MovieId == i_SelectedMovieId).ToList();
        }
    }
}
